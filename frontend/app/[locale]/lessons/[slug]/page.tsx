...
import { LessonGrammarSpotlight } from "@/components/lessons/lesson-grammar-spotlight";
    ...
    return (
      <div className="pt-28 md:pt-36">
        <article className="rounded-[2.5rem] border border-amber-950/10 bg-white/85 p-8 shadow-card">
          ...
          {lesson.unit_number <= 22 ? (
            <div className="mt-8">
              <LessonGrammarSpotlight locale={params.locale as "vi" | "en" | "ja"} unitNumber={lesson.unit_number} />
            </div>
          ) : null}
          {lesson.unit_number <= 22 ? null : lesson.slug === "n5-greetings-and-self-introduction" ? null : (
            <LessonSections locale={params.locale} sections={lesson.sections} />
          )}
        </article>
      </div>
    );
